...
class groupbuy_model extends CI_Model{
	 ...
	 * 提交用户订单
	 * @author Hewr
	 * @param shopID shopname username cargoList totalMoney
	 */
	function submitOrder($shopid, $shopname, $username, $list, $amount) {
		if (count($this->getShopById($shopid)) == 0) return;
		$sql = "INSERT INTO `groupbuy_order`(`shopid`, `shopname`, `username`, `list`, `amount`, `createtime`, `userID`) VALUES(?,?,?,?,?,?,?)";
		$res = $this->db->query($sql,array($shopid,$shopname,$username,json_encode($list),$amount,date("Y-m-d H:i:s", mktime()), $_SESSION['userID'])) or die(mysql_error());
	}
	...
	function getAllOrders($username) {
		$sql = "SELECT `id`,`shopid`,`list`,`amount`,`shopname`,`createtime` FROM `groupbuy_order` WHERE `username`=? and `del`=0 ORDER BY -`id`";
